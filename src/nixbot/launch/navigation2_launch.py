...
def generate_launch_description():
    ...
    nav2_yaml = os.path.join(get_package_share_directory('nixbot'), 'config', 'nav2_params.yaml')
    rviz_config_dir = os.path.join(get_package_share_directory('nixbot'), 'config', 'nav2_default_view.rviz')
    map_file = os.path.join(get_package_share_directory('nixbot'), 'config', 'turtlebot3_wolrd.yaml')
